...
def setup_mocks(mocker, mounts, state_files, is_pid_running=True):
    ...
    mocker.patch('cpfs_nfs_common.get_current_local_nfs_mounts', return_value=mounts)
    mocker.patch('watchdog.get_state_files', return_value=state_files)
    mocker.patch('cpfs_nfs_common.is_pid_running', return_value=is_pid_running)
    mocker.patch('cpfs_nfs_common.mark_as_unmounted', return_value=state)
...
def test_no_state_files(mocker):
    ...
    watchdog.check_alinas_mounts(wd, GRACE_PERIOD)
...
def test_malformed_state_file(mocker, tmpdir):
    ...
    watchdog.check_alinas_mounts(wd, GRACE_PERIOD, state_file_dir)
