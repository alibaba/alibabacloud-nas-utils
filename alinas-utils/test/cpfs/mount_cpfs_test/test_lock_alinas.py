...
def lock_alinas(state_dir, expected_success):
    lock_file = os.path.join(state_dir, cpfs_nfs_common.ALINAS_LOCK)
...
def test_lock_alinas_conflict1(tmpdir):
    ...
    with cpfs_nfs_common.lock_alinas(state_dir):
        assert os.path.exists(os.path.join(state_dir, cpfs_nfs_common.ALINAS_LOCK))
