...
def test_local_nfs_mount(tmpdir):
    fstab = [
        MOUNT_FMT_LINE.format(address='alinas-127.0.0.1', mountpoint='/mnt', fs_type='nfs4', options=DEFAULT_OPTS),
        MOUNT_FMT_LINE.format(address='alinas-127.0.1.1', mountpoint='/mnt', fs_type='nfs4', options=DEFAULT_OPTS),
        MOUNT_FMT_LINE.format(address='alinas-127.0.2.1', mountpoint='/mnt', fs_type='nfs4', options=DEFAULT_OPTS),
        MOUNT_FMT_LINE.format(address='alinas-127.0.0.2', mountpoint='/mnt', fs_type='nfs', options=DEFAULT_OPTS),
        MOUNT_FMT_LINE.format(address='alinas-127.0.1.2', mountpoint='/mnt', fs_type='nfs', options=DEFAULT_OPTS),
        MOUNT_FMT_LINE.format(address='alinas-127.0.2.2', mountpoint='/mnt', fs_type='nfs', options=DEFAULT_OPTS),
    ]
    mount_file = _create_mount_file(tmpdir, fstab)
    ...
    assert len(mounts) == 6
    assert 'alinas-127.0.0.1' in mounts
    assert 'alinas-127.0.1.1' in mounts
    assert 'alinas-127.0.2.1' in mounts
    assert 'alinas-127.0.0.2' in mounts
    assert 'alinas-127.0.1.2' in mounts
    assert 'alinas-127.0.2.2' in mounts


def test_local_nfs_mount_noresvport(tmpdir):
    mount_file = _create_mount_file(tmpdir, [MOUNT_FMT_LINE.format(address='alinas-127.0.0.1', mountpoint='/mnt',
