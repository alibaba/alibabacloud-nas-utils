...
def _test_main(mocker, root=True):
    ...
    config = watchdog.SafeConfig(config, None, None)

    parse_argument_mock = mocker.patch('watchdog.parse_arguments')
    read_config_mock = mocker.patch('watchdog.read_config', return_value=config)
    bootstrap_logging_mock = mocker.patch('watchdog.bootstrap_logging')
    check_alinas_mounts_mock = mocker.patch('watchdog.check_nfs_mounts')
    ...
    env_monitor = mock.MagicMock()
    env_monitor_mock = mocker.patch('watchdog.EnvMonitor', return_value=env_monitor)
    ...
    if root:
        ...
        env_monitor.stop.assert_called_once()
        env_monitor_mock.assert_called_once()
