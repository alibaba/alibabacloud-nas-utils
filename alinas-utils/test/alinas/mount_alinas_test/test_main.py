...
def _test_main(mocker, mode, root=True):
    ...
    mount_alinas.TLS_ENABLED = True
    read_config_mock = mocker.patch('mount_alinas.read_config', return_value='config')
    bootstrap_logging_mock = mocker.patch('mount_alinas.bootstrap_logging')
    check_unsupported_options_mock = mocker.patch('mount_alinas.check_unsupported_options')
    get_init_system_mock = mocker.patch('mount_alinas.get_init_system', return_value='init')
    check_network_status_mock = mocker.patch('mount_alinas.check_network_status')
    parse_arguments_mock = mocker.patch('mount_alinas.parse_arguments',
                                        return_value=('dns', 'fs', 'path', 'mp', options))
    mount_tls_mock = mocker.patch('mount_alinas.mount_tls')
    mount_nfs_directly_mock = mocker.patch('mount_alinas.mount_nfs_directly')
