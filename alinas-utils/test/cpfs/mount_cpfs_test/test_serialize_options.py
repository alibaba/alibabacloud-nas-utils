...
def test_serialize_options_invalid():
    for i in mount_cpfs.ALINAS_ONLY_OPTIONS:
        options = {i: 10}
        assert mount_cpfs.serialize_options(options) == ''


def test_serialize_options_valid():
    options = {'rw': None}
    text = mount_cpfs.serialize_options(options)
    assert text == 'rw'
...
def test_serialize_options_mix():
    ...
    assert mount_cpfs.serialize_options(options) == 'retrans=1'
