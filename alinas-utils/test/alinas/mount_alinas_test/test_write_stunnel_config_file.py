...
def _get_expected_alinas_config(proxy=PROXY, port=PORT, dns_name=DNS_NAME,
                                verify=mount_alinas.DEFAULT_STUNNEL_VERIFY_LEVEL,
                                check_cert_hostname=True,
                                check_cert_validity=True,
                                disable_libwrap=True):

    expected_alinas_config = dict(mount_alinas.STUNNEL_ALINAS_CONFIG)
...
def _test_check_cert_hostname(mocker, tmpdir, stunnel_check_cert_hostname_supported, stunnel_check_cert_hostname,
    ...
    ca_mocker = mocker.patch('mount_alinas.add_stunnel_ca_options')

    config_file = mount_alinas.write_stunnel_config_file(
    ...
    _validate_config(config_file, mount_alinas.STUNNEL_GLOBAL_CONFIG,
...
def _test_write_stunnel_config_file(mocker, tmpdir):
    ...
    config_file = mount_alinas.write_stunnel_config_file(_get_config(mocker),
                                                         state_file_dir,
                                                         LOCAL_DNS,
                                                         PROXY,
                                                         PORT,
    ...
    _validate_config(config_file, mount_alinas.STUNNEL_GLOBAL_CONFIG, _get_expected_alinas_config())
...
def test_write_stunnel_config_with_debug(mocker, tmpdir):
    ...
    config_file = mount_alinas.write_stunnel_config_file(_get_config(mocker, stunnel_debug_enabled=True),
                                                         state_file_dir,
                                                         LOCAL_DNS,
                                                         PROXY,
                                                         PORT, DNS_NAME, VERIFY_LEVEL)
    ca_mocker.assert_called_once()

    expected_global_config = dict(mount_alinas.STUNNEL_GLOBAL_CONFIG)
    expected_global_config['debug'] = 'debug'
    expected_global_config['output'] = os.path.join(
        mount_alinas.LOG_DIR,
        '%s.stunnel.log' % mount_alinas.compose_local_dns_with_uuid(FS_ID, PROXY)[0])

    _validate_config(config_file, expected_global_config, _get_expected_alinas_config())
...
def test_write_stunnel_config_check_cert_hostname_not_supported_flag_set_true(mocker, capsys, tmpdir):
    ...
    with pytest.raises(SystemExit) as ex:
        ...
        mount_alinas.write_stunnel_config_file(conf, str(tmpdir), LOCAL_DNS, PROXY, PORT, DNS_NAME,
                                               VERIFY_LEVEL)
...
def test_write_stunnel_config_with_verify_level(mocker, tmpdir):
    ...
    verify = 0
    config_file = mount_alinas.write_stunnel_config_file(_get_config(mocker, stunnel_check_cert_validity=True),
                                                         str(tmpdir),
                                                         LOCAL_DNS,
                                                         PROXY,
...
def _test_disable_libwrap(mocker, tmpdir, system_release='unknown', disable_libwrap=True):
    mocker.patch('mount_alinas.add_stunnel_ca_options')
    mocker.patch('mount_alinas.get_system_release_version', return_value=system_release)
    ...
    _validate_config(config_file, mount_alinas.STUNNEL_GLOBAL_CONFIG,
                     _get_expected_alinas_config(disable_libwrap=disable_libwrap))


def test_write_stunnel_config_for_rhel8_disable_libwrap(mocker, tmpdir):
    _test_disable_libwrap(mocker, tmpdir,
...
def test_write_stunnel_config_for_non_rhel8_enable_libwrap(mocker, tmpdir):
    _test_disable_libwrap(mocker, tmpdir,
                          system_release='Alibaba Cloud Linux (Aliyun Linux) release 2.1903 LTS (Hunting Beagle)',
                          disable_libwrap=True)
