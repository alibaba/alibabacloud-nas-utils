...
def _get_config():
    ...
    config.set(mount_alinas.CONFIG_SECTION, 'proxy_port', str(8888))
...
def test_choose_proxy_addr_first_try(mocker):
    ...
    ip, port = mount_alinas.choose_proxy_addr(_get_config())
...
def test_choose_proxy_addr_never_succeeds(mocker, capsys):
    ...
    with pytest.raises(SystemExit) as ex:
        mount_alinas.choose_proxy_addr(_get_config())
