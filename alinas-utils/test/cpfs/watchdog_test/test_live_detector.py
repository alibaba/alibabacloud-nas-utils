...
STATE_FILE = 'cpfs-123456abc.127.0.0.1'


def _dump_state(tmpfile, state):
    state[watchdog.STATE_SIGN] = cpfs_nfs_common.sign_state(state)
...
def test_ping_nothing(mocker, tmpdir):
    ...
    detector = watchdog.LiveDetector(MagicMock(), MagicMock(), fm, str(tmpdir))
...
def test_ping_refresh_remove(mocker):
    ...
    fm = cpfs_nfs_common.StateFileManager()
    detector = watchdog.LiveDetector(MagicMock(), MagicMock(), fm)
    try:
        detector.start()
        detector._volumes = {
            'fs1': None,
...
# there are lots of temporal dependencies in this testcase, we should refactor it
def test_ping_timeout(mocker, tmpdir):
    event = threading.Event()
    statvfs_mock = mocker.patch('os.statvfs', side_effect=lambda _: event.wait())
    eventbus = watchdog.EventBus()
    _dump_state(tmpdir.join(STATE_FILE), {
        'timeo': 4,
        'mountpoint': '/mnt'
    })
    fm = cpfs_nfs_common.StateFileManager()
    detector = watchdog.LiveDetector(MagicMock(), eventbus, fm, str(tmpdir))
    detector.PING_TIMEOUT_IN_SEC = 5
    try:
        detector.start()
        detector.refresh({STATE_FILE: None})

        time.sleep(detector.PING_TIMEOUT_IN_SEC + 3)

        cmds = eventbus.fetch_and_remove()
        assert len(cmds) == 1
        assert isinstance(cmds[0], watchdog.RestartCommand)
        event.set()
        ...
        time.sleep(4)
        assert statvfs_mock.call_count != 0
    finally:
        event.set()
        detector.stop()
