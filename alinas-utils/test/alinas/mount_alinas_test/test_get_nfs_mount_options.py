...
def test_get_default_nfs_mount_options():
    nfs_opts = mount_alinas.get_nfs_mount_options({})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_nfsvers_to_vers():
    nfs_opts = mount_alinas.get_nfs_mount_options({'nfsvers': 4.0})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)

    assert 'nfsvers=4.0' not in nfs_opts
    assert 'vers=4.0' not in nfs_opts
    assert 'vers=4' in nfs_opts
    assert 'minorversion=0' in nfs_opts


def test_override_nfs_version_alternate_option():
    nfs_opts = mount_alinas.get_nfs_mount_options({'vers': 4.0})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_override_rsize():
    nfs_opts = mount_alinas.get_nfs_mount_options({'rsize': 1})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_override_wsize():
    nfs_opts = mount_alinas.get_nfs_mount_options({'wsize': 1})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_override_recovery_soft():
    nfs_opts = mount_alinas.get_nfs_mount_options({'soft': None})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_override_timeo():
    nfs_opts = mount_alinas.get_nfs_mount_options({'timeo': 1})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_override_retrans():
    nfs_opts = mount_alinas.get_nfs_mount_options({'retrans': 1})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_tls_without_proxy(capsys):
    with pytest.raises(SystemExit) as ex:
        mount_alinas.get_nfs_mount_options({'tls': None})
...
def test_tls():
    nfs_opts = mount_alinas.get_nfs_mount_options({'tls': None, 'proxy': '127.0.0.1', 'proxy_port': 9000})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_proxy_without_proxyport(capsys):
    with pytest.raises(SystemExit) as ex:
        nfs_opts = mount_alinas.get_nfs_mount_options({'proxy': '127.0.0.1'})
...
def test_proxy_with_port(capsys):
    with pytest.raises(SystemExit) as ex:
        mount_alinas.get_nfs_mount_options({'proxy': '127.0.0.1', 'port': 3030, 'proxy_port': 8888})
...
def test_proxy():
    nfs_opts = mount_alinas.get_nfs_mount_options({'proxy': '127.0.0.1', 'proxy_port': 8000})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_proxy_v3():
    nfs_opts = mount_alinas.get_nfs_mount_options({'proxy': '127.0.0.1', 'proxy_port': 8000, 'vers': 3})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
...
def test_bare_v3():
    nfs_opts = mount_alinas.get_nfs_mount_options({'vers': 3})
    nfs_opts = mount_alinas.serialize_options(nfs_opts)
